...
def process_configs(config: Union[CommonArgs, Munch], fold: int):
    ...
    config.log_path = opj(log_dirs, config.log_file + f'{fold}.log')
    ...
    config.test_ckpt = opj(config.ckpt_dir, f'best{fold}.ckpt')
    config.id_test_ckpt = opj(config.ckpt_dir, f'id_best{fold}.ckpt')
